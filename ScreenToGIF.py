...
class AppConfig:
    ...
    def _ensure_output_directory(self):
        ...
        default_output = os.path.join(program_dir, "ScreenToGIF_Recordings")
        
        # 如果当前输出目录为空，使用默认值
        if not self.output_directory:
            self.output_directory = default_output
        else:
            # 检查输出目录是否仍然是程序目录下的子目录
            # 如果不是，也重置为默认值
            try:
                # 获取输出目录的相对路径（相对于程序目录）
                rel_path = os.path.relpath(self.output_directory, program_dir)
    ...
    def load_from_file(cls, filepath: str) -> 'AppConfig':
        ...
        try:
            if os.path.exists(filepath):
                ...
                config = cls(**data)
                # 加载后也确保目录正确
                config._ensure_output_directory()
                return config
        except Exception as e:
            print(f"加载配置文件失败: {e}")
        return cls()
...
class ScreenToGIFMainWindow(QMainWindow):
    ...
    def _ensure_output_directory(self):
        ...
        # 检查当前输出目录
        current_output = self.config.output_directory
        
        # 如果当前输出目录为空或不在程序目录下，使用默认值
        if not current_output:
            self.config.output_directory = default_output
        else:
            try:
                # 获取输出目录的相对路径（相对于程序目录）
                rel_path = os.path.relpath(current_output, program_dir)
                if rel_path.startswith('..') or os.path.isabs(rel_path):
                    # 如果不在程序目录下，重置为默认值
                    self.config.output_directory = default_output
                    print(f"输出目录已重置为: {default_output}")
            except:
                # 如果发生任何错误，也重置为默认值
                self.config.output_directory = default_output
